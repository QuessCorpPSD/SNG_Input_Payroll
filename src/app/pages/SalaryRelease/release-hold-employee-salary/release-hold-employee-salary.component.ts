...
import { ReleaseHoldEmployeSalaryService } from '../../../Service/SalaryRelease/release-hold-employe-salary.service';
...
@Component({
  selector: 'app-release-hold-employee-salary',
  standalone: true,
  imports: [
    CompanyallComponent,
    CommonModule,
  ...
  templateUrl: './release-hold-employee-salary.component.html',
  styleUrl: './release-hold-employee-salary.component.css',
  providers: [
    { provide: Common_TOKEN, useClass: ReleaseHoldEmployeSalaryService }
  ]
})
export class ReleaseHoldEmployeeSalaryComponent {
  ...
  displayedColumns: string[] = ['select', 'serial_No', 'company_Code', 'company_Name', 'employee_Code', 'employee_Name', 'purpose', 'inpuT_NO', 'batchID', 'net_Pay', 'salary_Hold_Type'];
  ...
  constructor(@Inject(Common_TOKEN) private leave: ReleaseHoldEmployeSalaryService, private _sessionStoreage: SessionStorageService, private decry: EncryptionService) { }
  ...
  handleCompanyEvent(event: any) {
    ...
    console.log('Company selected:', this.companyId, this.selectedCompanyCode);
  }
  ...
  handlePayperiodEvent(payperiod: Payperiodclass) {
    ...
    console.log('Pay period selected:', this.payPeriodId, this.selectedPP);
  }
  ...
  ngOnInit(): void {
    ...
    if (json) {
      ...
      console.log('User details loaded:', this.userdetail);
    } else {
      console.warn('UserProfile not found in session storage');
    }
  }

  HoldReleaseEmployeSearch() {
    this.isLoading = true;
    ...
    this.leave.HoldReleaseEmployeSearch(Payload).subscribe({
      next: (res) => {
        this.isLoading = false;
        ...
        if (this.salaryRelease && this.salaryRelease.length > 0) {
          ...
          this.displayedColumns = ['select', 'serial_No', 'company_Code', 'company_Name', 'employee_Code', 'employee_Name', 'purpose', 'inpuT_NO', 'batchID', 'net_Pay', 'salary_Hold_Type'];
        } else {
  ...
  exportToExcel(): void {
    ...
    this.leave.HoldReleaseEmployeSearch(exportPayload).subscribe({
      next: (res) => {
  ...
  submitUploadedData() {
    ...
    this.leave.UploadRequest(this.formData).subscribe(

...
import { BankDetailsGrid, BankDetailsImportGrid } from '../../../Models/SalaryRelease/BankAccount.model';
...
export class BankDetailsComponent implements OnInit {
  ...
  SelectedtoDate: any;
  dataSource = new MatTableDataSource<BankDetailsGrid>([]);
  dataSourceImport = new MatTableDataSource<BankDetailsImportGrid>([]);
  ...
  displayedColumns: string[] = [
    'Employee_Id', 'Bank_Name', 'Bank_Account_Number', 'IFSC_Code',
    'Bank_Account_Name'
  ];

  displayedColumnsImport: string[] = [
    'select', 'Employee_Id', 'Bank_Name', 'Bank_Account_Number', 'IFSC_Code',
    'Bank_Account_Name'];
  ...
  selection = new SelectionModel<BankDetailsGrid>(true, []);
  ...
  toggleRow(row: BankDetailsGrid) {
    this.selection.toggle(row);
  }

  selectionImport = new SelectionModel<BankDetailsImportGrid>(true, []);
  ...
  toggleRowImport(row: BankDetailsImportGrid) {
    this.selection.clear();
  ...
  BindDashBoard(companyCode: string) {
    ...
    this.bankaccountservice.BankDetailsSearch(Company_Id).subscribe({
      next: res => {
  ...
  onFileChange(event: any): void {
    ...
    reader.onload = (e: any) => {
      try {
        ...
        const jsonDataimport = XLSX.utils.sheet_to_json<BankDetailsImportGrid>(sheet);
  ...
  callImportApi(rows: any) {
    ...
    this.bankaccountservice.BankDetailsUpload(payload).pipe(
      finalize(() => {
  ...
  onTemplateClick() {
       ...
       const baseHeaders = ["Employee_Id", "Bank_Name","Bank_Account_Number","IFSC_Code","Bank_Account_Name"];
       const data: any[][] = [baseHeaders];
       const ws: XLSX.WorkSheet = XLSX.utils.aoa_to_sheet(data);
   
       const wb: XLSX.WorkBook = XLSX.utils.book_new();
   
       XLSX.utils.book_append_sheet(wb, ws, "Bank_Details");
  ...
  onExportClick() {
    ...
    var Company_id = this.companyUI.companyId;
    var QZoneUserName = "123"
    this.bankaccountservice.BankDetailsSearch(Company_id)
      .pipe(
        finalize(() => {
          this.isLoading = false;   // always runs
        })
        ...
        next: res => {
          ...
          // FileSaver.saveAs(data, `Bank_Details_${dateTime}.xlsx`);
          XLSX.writeFile(workbook, `Bank_Details_${dateTime}.xlsx`);
        },
        error: err => {
          console.error('Error fetching data:', err.message);
          this.isLoading = false;

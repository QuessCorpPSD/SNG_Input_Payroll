...
export const Common_TOKEN = new InjectionToken<IUpfrontApproval>('Common_TOKEN');
...
import { UpfrontApprovalService } from '../../../Service/SalaryRequestNew/UpfrontApproval';
import { IUpfrontApproval } from '../../../Repository/SalaryRequestNew/IUpfrontApproval';
import { UpfrontApprovalGrid, UpfrontApprovalImportGrid } from '../../../Models/SalaryRelease/UpfrontApproval.model';


@Component({
  selector: 'Upfrontapproval',
  standalone: true,
  ...
  templateUrl: './upfront-approval.component.html',
  styleUrl: './upfront-approval.component.css',
  providers: [
    { provide: Common_TOKEN, useClass: UpfrontApprovalService }]
})
export class UpfrontApprovalComponent implements OnInit {

  companyUI: any;
  payperiodUI: any;
  SelectedfromDate: any;
  SelectedtoDate: any;
  dataSource = new MatTableDataSource<UpfrontApprovalGrid>([]);
  dataSourceImport = new MatTableDataSource<UpfrontApprovalImportGrid>([]);
  ...
  displayedColumns: string[] = [
    'Select', 'Company_Code', 'Client_Name', 'Invoice_Number',
    'Payperiod', 'Map_Name', 'TotalEmployees', 'Amount', 'Net_Amount'
  ];
  ...
  TEMPLATE_HEADERS: Record<string, string[]> = {
    Release: [
      'FLAG', 'Slno', 'Invoice_Id', 'Company_Code', 'Client_Name', 'Invoice_Number',
      'Payperiod', 'Map_Name', 'TotalEmployees', 'Amount', 'Net_Amount'
    ]
  };
  ...
  constructor(
    ...
    @Inject(Common_TOKEN) private upfrontapprovalservice: IUpfrontApproval,
  ...
  selection = new SelectionModel<UpfrontApprovalGrid>(true, []);
  ...
  toggleRow(row: UpfrontApprovalGrid) {
    this.selection.toggle(row);
  }

  selectionImport = new SelectionModel<UpfrontApprovalImportGrid>(true, []);
  isAnyFilteredRowSelectedImport(): boolean {
    return this.selectionImport.selected.some(sel =>
      this.dataSourceImport.filteredData.some(row => row['Invoice_Number'] === sel['Invoice_Number'])
    );
  }
  isAllSelectedImport() {
  ...
  toggleRowImport(row: UpfrontApprovalImportGrid) {
    this.selection.clear();
  ...
  BindDashBoard(companyCode: string, payperiod: string) {
    ...
    const payload = {
      "COMPANY_ID": String(this.companyUI.companyId),
      "Pay_FrequencyId": String(this.payperiodUI.payfrequencyid),
    ...
    this.upfrontapprovalservice.UpfrontViewInvoice(payload).subscribe({
      next: res => {
  ...
  onFileChange(event: any): void {
    ...
    reader.onload = (e: any) => {
      try {
        ...
        const jsonDataimport = XLSX.utils.sheet_to_json<UpfrontApprovalImportGrid>(sheet);
  ...
  onTemplateClick() {
    ...
    this.upfrontapprovalservice.UpfrontViewInvoice(payload).pipe(
      finalize(() => {
  ...
  onExportClick() {
    ...
    this.upfrontapprovalservice.UpfrontViewInvoice(payload)
      .pipe(
        finalize(() => {
          this.isLoading = false;   // always runs
        })
        ...
        next: res => {
          ...
          const workbook: XLSX.WorkBook = {
            Sheets: { 'Upfront Approval': worksheet },

...
export class SalaryReissueComponent implements OnInit {

  dataSource = new MatTableDataSource<SalaryReissueGrid>([]);
  ...
  selection = new SelectionModel<SalaryReissueGrid>(true, []);
  ...
  toggleRow(row: SalaryReissueGrid) {
  ...
  callAllSalaryApi(rows: any) {
    ...
    const payload = {
      ...
      ReissueRequestList: rows.map(r => ({
        Bank_Invoice_Id: String(r.Bank_Invoice_Id),
    ...
    this.releaseservice.ReissueRequest(payload).pipe(
      finalize(() => {
        this.isLoading = false;   // always runs
      })
    ).subscribe({
      next: res => {
        ...
        if (validations.length > 0) {
          this.downloadValidationExcel(validations, "Salary_Reissue_Request_Validations");
        }
      },
    ...
    this.releaseservice.SearchAllReleaseRequest(Company_Id, Pay_Period_Id, Flag,
      InvoiceNo, QZoneUserName).pipe(
        ...
        next: res => {
          ...
          const workbook: XLSX.WorkBook = {
            Sheets: { 'Salary Reissue': worksheet },

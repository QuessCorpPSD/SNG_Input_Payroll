...
import { DBTReleaseImportGrid } from '../../../Models/SalaryRelease/DBTReleaseImport';
...
export class DBTReleaseComponent implements OnInit {

  dataSource = new MatTableDataSource<DBTReleaseGrid>([]);
  dataSourceImport = new MatTableDataSource<DBTReleaseImportGrid>([]);
  ...
  displayedColumns: string[] = [
    ...
    'Bank_Account_Number', 'Bank_Name', 'IFSC_Code', 'DBT_Hold_Amount', 'Net_Pay', 'Ekyc_Status'
  ];
  ...
  selection = new SelectionModel<DBTReleaseGrid>(true, []);
  ...
  toggleRow(row: DBTReleaseGrid) {
    this.selection.toggle(row);
  }

  selectionImport = new SelectionModel<DBTReleaseImportGrid>(true, []);
  ...
  toggleRowImport(row: DBTReleaseImportGrid) {
    this.selection.clear();
  ...
  onFileChange(event: any): void {
    ...
    reader.onload = (e: any) => {
      try {
        ...
        const jsonDataimport = XLSX.utils.sheet_to_json<DBTReleaseImportGrid>(sheet);
  ...
  callAllSalaryApi(rows: any) {
    ...
    const payload = {
      ...
      DBTReleaseList: rows.map(r => ({
        InvoiceNumber: String(r.Invoice_No),
        EmployeeCode: String(r.Employee_Code),
        DBTReleaseAmount: String(r.DBT_Hold_Amount),
        SalaryType: String(r.SalaryType)
      }))
    };

    this.releaseservice.DBTHoldRelease(payload).pipe(
      finalize(() => {
  ...
  callImportApi(rows: any) {
    ...
    const payload = {
      ...
      DBTReleaseList: rows.map(r => ({
        ...
        DBTReleaseAmount: String(r.PartialReleaseAmount),
        SalaryType: String(r.SalaryType)
      }))
    };
  ...
  onTemplateClick() {
    ...
    var Flag = 'DBT Hold Salary Release';

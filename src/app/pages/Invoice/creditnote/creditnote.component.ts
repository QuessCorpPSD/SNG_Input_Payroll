...
@Component({
  ...
  imports: [CommonModule,
    ...
    MatButtonModule,
    CompanyComponent,
    MatTooltipModule, PayPeriodComponent],
...
export class CreditnoteComponent {
  ...
  SearchClick() {
    ...
    this.creditService.CreditNoteSearch(payload).subscribe({
      next: (res: any) => {
        console.log(res.Data);
  ...
  DownloadTemplate() {
    ...
    // FileSaver.saveAs(blob, `CreditNoteRequest_Template.xlsx`);
    XLSX.writeFile(wb, 'CreditNoteRequest_Template.xlsx');
  }
  ...
  onFileChange(event: any): void {
    ...
    if (this.excelFile) {
      ...
      this.creditService.UploadCreditNoteRequest(formData).subscribe({
        next: (res) => {
          console.log(res);
          this.datatable = res.Data;
          console.table(this.datatable);
          if (this.datatable && Array.isArray(this.datatable) && this.datatable.length > 0) {
            this.downloadExcel(this.datatable, "CreditNoteRequest_Validations");
            this.isLoading = false;
          } else {
  ...
  Export() {
    ...
    this.creditService.ExportCreditNoteRequest(payload).subscribe({
      next: (res: any) => {
        console.log(res.Data);
        this.isLoading = false;

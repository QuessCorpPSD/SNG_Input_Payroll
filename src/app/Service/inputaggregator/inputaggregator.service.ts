...
export class InputaggregatorService implements IInputaggregator {
  ...
  QuessAttendanceAttributeClient(companyId: number, siteid: number): Observable<APIResponse> {
    return this.http.get<APIResponse>(
      this.env.apiUrl + `Attendance/QuessAttendanceAttributeClient/${companyId}/${siteid}`);
  }
